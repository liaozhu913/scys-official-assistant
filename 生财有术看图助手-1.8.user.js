...
(function () {
    ...
    async function buildLarkDownloadBundle() {
        ...
        const renderedImages = getLarkRenderedImagesFromDom();
        let imageIndex = 0;
        ...
        for (const block of blocks.slice(imageIndex)) {
            const image = block?.snapshot?.image || {};
            if (image.__scysArchivePath) continue;
            const sources = await fetchLarkImageSources(block);
            const src = getLarkImageCachedUrl(block) || sources?.src || sources?.originSrc;
            if (!isPreviewableImageUrl(src)) continue;
            await addImageEntry(src, image, block);
        }
